...
::: callout-note
Causal approaches define effects using the counterfactual framework,
before estimating the effects using statistical methods.
:::
...
-   Causal Assumptions
    ...
    Imagine if we don't have anyone with A = 0 among those with L=1
    (e.g. long education), then we cannot estimate the conditional
...
Here, we see that physical activity (a) is associated with lower BMI
(m).
...
uniformly in the population but the exposure (being physically active),
...
The PNDE is how much the outcome, pulse, would change if the exposure
(being physically active), was set at 1 versus 0 but for each individual
the mediator was kept at the level it would have taken, for that
individual, in the absence of the exposure.

This means: how pulse rate would change if the exposure (being
physically active) (a=1) was set at 1 versus 0 (everybody was physically
active), while for each individual the mediator (BMI) was kept at the
level it would naturally be in the **absence** of physical
activity(**a=0**).
...
Here, the value of M is enabled to act (as opposed to what happens for
the PNDE). Here we see a slightly lower pulse rate estimate, this means
we're measuring the direct effect of physical activity on pulse rate
while allowing BMI to be at the level it would naturally be with
physical activity (**a=1**).
...
Note that the exposure has to have an effect on M otherwise the TNIE
will be zero.
...
BMI?" In other words, the effect that the exposure would have had if
'its only action were to cause the mediator' (i.e., the portion of the
effect for which mediation is ''sufficient'')" @wang_g-computation_2015.
